var myDivClassName = "DivDocControl";
var myDivStatusIDName = "changeDocStatus";
var sFunctionName = "DocControlServlet";
var sFunctionUrl = "inmethod/doc/" + sFunctionName;
...
function showDocMessage(message, color) {
...
$(function () {
    ...
    if( !(sCatalogValue!==null && sCatalogValue!== "undefined"))
       ...
       $.when(
            ...
            $.getJSON("inmethod/auth/AuthenticationControlServlet", {"FlowID": "getDocCatalogSelection",  "Catalog": sCatalogValue}),
                        ...
                        $.getJSON(sFunctionUrl, {
                            ...
                            if (typeof d.DeleteResult !== "undefined") {
                                if (d.DeleteResult == "OK") {
                                    ...
                                    showDocMessage("Delete Success", "green");
                                } else {
                                    result.reject(new Error("delete failed"));
                                    showDocMessage("Delete Fail", "red");
                                }
                            } else {
                                result.reject(new Error("delete failed"));
                                showDocMessage("Delete Fail", "red");
                        ...
                        if(   ItemData.UploadPath!== null  &&  ItemData.UploadPath !== undefined){
	                      ...
	                      if(  ! ( ItemData.UploadType!==null &&   ItemData.UploadType!==undefined &&  ItemData.UploadType!=='')
	                           || 
                                ...
                                showDocMessage("Insert Fail , file name or file type can not empty", "red");
                               return   result.promise();
	                      }
                        } else{
	                      if(  ( ItemData.UploadType!==null &&   ItemData.UploadType!==undefined &&  ItemData.UploadType!=='')
	                           || 
		                         ...
		                         result.reject(new Error("add failed"));
                                showDocMessage("Insert Fail , file name or file type must be empty", "red");
                               return   result.promise();
	                      }
                        }
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                showDocMessage("Insert Success! Refresh whote page if modify data!", "green");
                       //         console.log(data)
                                return result.promise();
                            },
                            error: function (err) {
                                result.reject(new Error("add failed"));
                                showDocMessage("Insert Fail", "red");
                                console.log(err);
                            }
                        });
                        ...
                        $.getJSON(sFunctionUrl, {
                            ...
                            if (typeof d.UpdateResult !== "undefined") {
                                if (d.UpdateResult == "OK") {
                                    result.resolve(ItemData);
                                    showDocMessage("Update Success", "green");
                                } else {
                                    result.resolve(lastPrevItem);
                                    showDocMessage("Update Fail", "red");
                                }
                            } else {
                                result.resolve(lastPrevItem);
                                showDocMessage("Update Fail", "red");
