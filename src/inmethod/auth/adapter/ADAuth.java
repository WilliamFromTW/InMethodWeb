...
import inmethod.auth.inter_face.IADAuth;
 ...
 * @author william
 */
public class ADAuth implements WebAuthentication{
   ...
   */
  public ADAuth(HttpServletRequest request,HttpServletResponse response) {
	...
	try {
    	...
    	String s[] = parseADSettings("AD_SETTINGS");
    	try {
    	  if( s!=null && s.length>=1) {
    		aAuths = new IADAuth[s.length];
    	    for(int i=0;i<s.length;i++) {
    	      String sAuthTemp[] = s[0].split(";");
    	      aAuths[i] = new IADAuth(sAuthTemp[0],sAuthTemp[1],sAuthTemp[2]);
    	    }
    	  }
    	}catch(Exception ee) {
    		ee.printStackTrace();
  ...
  public boolean login(String sUserID,String sUserPwd) throws Exception{
    ...
    if( session == null){     
      throw new Exception("inmthod.login , session is null");
    }
    System.out.println(sUserID);
...
//      session.setAttribute(AUTH_KEY,Crypt.MD5(sUserID));
  ...
  /**
   * logout .
   * @param sUserID user id to login
  ...
  /**
   * settings from tomcat web.xml , a set will include 3 parameter(domain,ip,search_base) , divide by ;   
   * example:
   * hlmt.com.tw;10.192.130.229;dc=hlmt,dc=com,dc=tw#hlmt.com.tw;10.192.130.229;dc=hlmt,dc=com,dc=tw
   * @return
   */
  private String[] parseADSettings(String sADSettings){
	  
    String s[] = Global.getInstance().getEnvirenment(sADSettings).split("#");
	return s;
  }
  ...
  public  boolean checkUserPassword(String sUserID,String sUserPwd) throws Exception {
    if( sUserID == null || sUserPwd == null) return false;
    try{
    	for( IADAuth aAuth:aAuths) {
          if(aAuth.authenticate(sUserID,sUserPwd)!=null)
            return true;
          
    	}
         return false;
    }catch(Exception ex){
      throw new Exception("ADAuth.checkUserPassword , try catch error ");
    }
  }
  ...
  public boolean setUserPassword(String sUserID, String sOldUserPwd , String sNewUserPwd) throws Exception{
    throw new Exception("M$ AD not support change password");
  }
...
//    return (String)session.getAttribute(AUTH_USER);
   ...
   */
  public boolean hasPermission(Vector aUserRoles,String sFunction) throws Exception{
    HashMap<String,UserRoles> aHM = getAuthorizedFunctionInfo(aUserRoles);
   ...
   * @param  sUserID
   * @param  sFunction function name
   * @return Object Function info object,null if no permission
   */
  public Object getAuthFunctionInfo(String sUserID,String sFunction) throws Exception{
   ...
   * </pre>
   * @param  sUserID
   * @return Vector Function info object,null if no Function be authorized
   */
...
//			System.out.println(" role set = " + sTempCondition);
	...
	//		System.out.println("getAuthFunctionInfo(String), Authorized Function hashmap size=" + aHM.size());
			...
			for (String key : keys) { 
			  // String value = (String)aHM.get(key);
				//System.out.println("dddddd"+sUserID+key);
				aFunInfo = (RoleAuthorizedPermission) getAuthorizedFunctionInfo(sUserID, key);
				//System.out.println(aFunInfo.getFunctionName());
				if (aFunInfo != null){
					//System.out.println(aFunInfo.getFunctionName());
  ...
  public boolean isLogin(String sUserID){
    ...
    if( sUserID.equals(sUserNameValue) && Crypt.MD5( sUserNameValue ).equals(sUserNameMd5) )
      ...
      return false;
/*
    if( session.getAttribute(AUTH_USER).equals(sUserID) )
      return true;
    else
      return false;
*/
   ...
   * @return HashMap size=0 if no function be auth
   */
			...
			if (aRS != null)
				while (aRS.next()) {
					if (aReturn.get(aRS.getString("function_name")) == null) {
						//System.out.println("getAuthFunction(Vector ),function_name=" + aRS.getString("function_name")+",roles="+aRS.getString("role_name"));
  ...
  public static void main(String a[]){
    try{
        ...
        IADAuth aAuth2 = new IADAuth("test2.com","192.168.1.2","OU=Taipei,dc=test2,dc=com");
       System.out.println("check ad");
       if(aAuth.authenticate("920405","xxxx")!=null)
         System.out.println("true");
       else{
         if(aAuth2.authenticate("tw001","123456789")!=null)
           System.out.println("true");
         else
...
public RoleAuthorizedPermission getAuthorizedFunctionInfo(String sUserID, String sFunction) throws Exception {
	...
	try(Connection aConn = DBConnectionManager.getWebDBConnection().getConnection()) {
		aUserRoles = getUserRoles(sUserID);
		//System.out.println(" getAuthFunctionInfo(String,String), aUserRoles #=" + aUserRoles.size()+",sFunction="+sFunction+",permission="+ !hasPermission(aUserRoles, sFunction));
		if (aUserRoles == null || aUserRoles.size() == 0 || !hasPermission(aUserRoles, sFunction))
			return null;
		for (int i = 0; i < aUserRoles.size(); i++) {
			sTempCondition += ",'" + ((UserRoles)aUserRoles.get(i)).getRoleName() + "'";
		...
		aRS = SQLTools.getInstance().Select(aConn, "ROLE_AUTHORIZED_PERMISSION,FUNCTION_INFO", "*",
				"ROLE_AUTHORIZED_PERMISSION.function_name=FUNCTION_INFO.function_name and ROLE_AUTHORIZED_PERMISSION.function_role in "
						+ sTempCondition + " and  ROLE_AUTHORIZED_PERMISSION.function_name='" + sFunction + "'");
		//System.out.println("fetch size="+aRS.getFetchSize());
		if (aRS != null)
			while (aRS.next()) {
				//System.out.println("getAuthFunctionInfo,Function=" + aRS.getString("function_name"));
				if (aFunInfo == null) {
